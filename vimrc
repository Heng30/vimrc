...
let NERDTreeIgnore = ['\.swp$', '\.o$', '\.un\~$', '\.git$', '\.pyc$', '\.svn$', '\.out$', '^%', 'target', 'zig-cache', 'zig-out']
...
" ranger 文件管理器, 没配置好, 不太好用(python 太坑了)
...
let g:ctrlp_custom_ignore = {
    \ 'dir':  '\v([\/]\.(git|hg|svn)$)|target|zig-cache|zig-out',
...
let g:EasyGrepFilesToExclude = "*.bak,*.a,*.o,*.pyc,*.lst,*.d,*.zip,*.so,*.gz,*.tar,*.dis,*.map,*.size,*.readelf,*.asm,*.E,ctags,*.sym,target,*.log,node_modules"
...
if has('nvim-0.4.4') || has('patch-8.2.0750')
    nnoremap <nowait><expr><C-f> coc#util#has_float() ? coc#util#float_scroll(1) : "\<C-f>"
    nnoremap <nowait><expr><C-a> coc#util#has_float() ? coc#util#float_scroll(0) : "\<C-a>"
endif
...
autocmd BufRead,BufNewFilE *.toml    set filetype=cfg
...
" 定义文件注释类型
autocmd FileType c,cpp,verilog,qml,slint setlocal commentstring=//\ %s
...
func! CODE_FORMAT()
    ...
    elseif &filetype == 'javascript' || &filetype == 'vue' || &filetype == 'typescript'
